...
def _monitor_get(mon_name, **connection_args):
    """
    Return a monitor ressource or None
    """
    nitro = _connect(**connection_args)
    if nitro is None:
        return None
    mon = NSLBMonitor()
    mon.set_monitorname(mon_name)
    try:
        mon = NSLBMonitor.get(nitro, mon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSLBMonitor.get() failed: %s", error)
        mon = None
    _disconnect(nitro)
    return mon

def monitor_exists(mon_name, **connection_args):
    """
    Checks if a monitor exists
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.monitor_exists 'monitorName'
    """
    mon = _monitor_get(mon_name, **connection_args)
    if mon is None:
        return False
    return True

def monitor_add(mon_name,mon_type,lrtm,resptimeout,interval,mon_send,mon_recv, **connection_args):
    """
    Add a new monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.monitor_add 'monitorName'
    """
    ret = True
    if monitor_exists(mon_name):
        return False
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    mon = NSLBMonitor()
    mon.set_monitorname(mon_name)
    mon.set_type(mon_type)
    mon.set_lrtm(lrtm)
    mon.set_resptimeout(resptimeout)
    mon.set_interval(interval)
    mon.set_recv(mon_recv)
    mon.set_send(mon_send)
    try:
        NSLBMonitor.add(nitro, mon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSLBMonitor.add() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def monitor_delete(mon_name, **connection_args):
    """
    Delete a new service group
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.monitor_delete 'monitorName'
    """
    ret = True
    mon = _monitor_get(mon_name, **connection_args)
    if mon is None:
    ...
    if nitro is None:
        return False
    try:
        NSLBMonitor.delete(nitro, mon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSLBMonitor.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def monitor_update(mon_name, **connection_args):
    """
    Update monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.monitor_update 'monitorName'
    """
    ret = True
    if monitor_exists(mon_name):
        return False
    ...
    mon = NSLBMonitor()
    mon.set_monitorname(mon_name)
    #mon.set_lrtm("ENABLED")
    try:
        NSLBMonitor.update(nitro, mon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSLBMonitor.update() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def sglbmonitorbinding_add(mon_name,sg_name, **connection_args):
    """
    Bind sg to a monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.sglbmonitorbinding_add 'monitorName' 'servicegroupName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    sgmon = NSServiceGroupLBMonitorBinding()
    sgmon.set_monitor_name(mon_name)
    sgmon.set_servicegroupname(sg_name)
    try:
        NSServiceGroupLBMonitorBinding.add(nitro, sgmon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSServiceGroupLBMonitorBinding.add() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def sglbmonitorbinding_get(sg_name, **connection_args):
    """
    Bind sg to a monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.sglbmonitorbinding_get 'servicegroupName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    sgmon.set_servicegroupname(sg_name)
    try:
        NSServiceGroupLBMonitorBinding.get(nitro, sgmon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSServiceGroupLBMonitorBinding.get() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def sglbmonitorbinding_delete(sg_name, **connection_args):
    """
    remove/delete sg to a monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.sglbmonitorbinding_delete 'servicegroupName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    sgmon.set_servicegroupname(sg_name)
    try:
        NSServiceGroupLBMonitorBinding.delete(nitro, sgmon)
    except NSNitroError as error:
        log.debug("netscaler module error - NSServiceGroupLBMonitorBinding.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvserver_get(csvs_name, **connection_args):
    """
    Add cs vs server
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.csvserver_get 'csvsName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    csvs = NSCSVServer()
    csvs.set_name(csvs_name)
    try:
        NSCSVServer.get(nitro, csvs)
    except NSNitroError as error:
        log.debug("netscaler module error - NSCSVServer.get() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvserver_add(csvs_name,csvs_type,csip,csvs_port,clttimeout, **connection_args):
    """
    Add cs vs server
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.csvserver_add 'csvsName' 'csvsType'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    csvs = NSCSVServer()
    csvs.set_name(csvs_name)
    csvs.set_servicetype(csvs_type)
    csvs.set_ipv46(csip)
    csvs.set_port(csvs_port)
    csvs.set_clttimeout(clttimeout)
    try:
        NSCSVServer.add(nitro, csvs)
    except NSNitroError as error:
        log.debug("netscaler module error - NSCSVServer.add() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvserver_delete(csvs_name, **connection_args):
    """
    remove/delete sg to a monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.csvserver_delete 'csvsName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    csvs.set_name(csvs_name)
    try:
        NSCSVServer.delete(nitro, csvs)
    except NSNitroError as error:
        log.debug("netscaler module error - NSCSVServer.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret
...
def lbvserver_add(lbvs_name,lbvs_type,persistype,backupvs,ip,port,clttimeout,timeout, **connection_args):
    """
    Add lb vs server
    CLI Example:
    .. code-block:: bash
...
def lbvserver_update(lbvs_name,persistencetype,rule,resrule, **connection_args):
    """
    update lb vs server
    CLI Example:
    .. code-block:: bash
    ...
    try:
        ...
        log.debug("netscaler module error - NSLBVServer.update() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret
...
def lbvserver_delete(lbvs_name, **connection_args):
    ...
    try:
        ...
        log.debug("netscaler module error - NSLBVServer.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret
...
def lbvsbindingtosg_get(lbvs_name, **connection_args):
    """
    Bind sg to a monitor
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.lbvsbindingtosg_get 'lbvsname'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    lbvssg.set_name(lbvs_name)
    try:
        NSLBVServerServiceGroupBinding.get(nitro, lbvssg)
    except NSNitroError as error:
...
def csvspolbind_add(csvs_name,pol_name,priority,priorityexpr, **connection_args):
    """
    Bind policy to a csvs
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.csvspolbind_add 'csvsname' 'policyName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    csvspol = NSCSVServerCSPolicyBinding()
    csvspol.set_name(csvs_name)
    csvspol.set_policyname(pol_name)
    csvspol.set_priority(priority)
    csvspol.set_gotopriorityexpression(priorityexpr)
    #csvspol.set_labeltype('REQUEST')
    try:
        NSCSVServerCSPolicyBinding.add(nitro, csvspol)
    except NSNitroError as error:
        log.debug("netscaler module error - NSCSVServerCSPolicyBinding.add() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvspolbind_delete(csvs_name,pol_name, **connection_args):
    """
    unBind policy to a csvs
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.csvspolbind_delete 'csvsname' 'policyName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    csvspol.set_policyname(pol_name)
    try:
        NSCSVServerCSPolicyBinding.delete(nitro, csvspol)
    except NSNitroError as error:
        log.debug("netscaler module error - NSCSVServerCSPolicyBinding.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def bindcsvstocert_add(csvs_name,cert_name, **connection_args):
    """
    Bind cert to a csvs
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.bindcsvstocert_add 'csvsname' 'certName'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
    ...
    try:
        NSSSLVServerSSLCertKeyBinding.add(nitro, csvspol)
    except NSNitroError as error:
...
def setsslvs_update(csvs_name,sslprofile, **connection_args):
    """
    Bind cert to a csvs
    CLI Example:
    .. code-block:: bash
        salt '*' netscaler.setsslvs_update 'csvsname'
    """
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    sslvs = NSSSLVServer()
    sslvs.set_vservername(csvs_name)
    sslvs.set_sslprofile(sslprofile)
    try:
        NSSSLVServer.update(nitro, sslvs)
    except NSNitroError as error:
        log.debug("netscaler module error - NSSSLVServer.update() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def nsip_get(ip, **connection_args):
    #ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    nsip = NSIP()
    nsip.set_ipaddress(ip)
    try:
        ret = NSIP.get(nitro,nsip)
    except NSNitroError as error:
        log.debug("netscaler module error - NSIP.get() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def nspatset_add(p_name, **connection_args):
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    nspat = NSPatset()
    nspat.set_patsetname(p_name)
    try:
        nspat.add(nitro, nspat)
    except NSNitroError as error:
        log.debug("netscaler module error - nspat.get() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvslbvs_bind(csvs_name,lbvs_name, **connection_args):
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    cslbvs = NSLBVServerCSVserverBinding()
    cslbvs.set_name(csvs_name)
    cslbvs.set_lbvserver(lbvs_name)
    try:
        NSLBVServerCSVserverBinding.add(nitro, cslbvs)
    except NSNitroError as error:
        log.debug("netscaler module error - cslbvs.add() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret

def csvslbvs_unbind(csvs_name,lbvs_name, **connection_args):
    ret = True
    nitro = _connect(**connection_args)
    if nitro is None:
        return False
    ...
    cslbvs.set_lbvserver(lbvs_name)
    try:
        NSLBVServerCSVserverBinding.delete(nitro, cslbvs)
    except NSNitroError as error:
        log.debug("netscaler module error - cslbvs.delete() failed: %s", error)
        ret = False
    _disconnect(nitro)
    return ret
